...
contract FactoryCloneContract is Ownable {
    ...
    constructor (address _USDC) {       
        ...
        //data
        bytes memory data = abi.encodeWithSignature("Initialize(address,address,address)",ImplementationAddress, _USDC,address(this));

        //owner
        address owner = msg.sender;

        //proxy
        emitterAddress = address(new ProxyContract(emitterImplementation,owner,data));        
    }

    /// @dev Function to change base Governor contract address
    /// @param _newImplementationAddress address of new Governor contract
    function changeDAOImplementation(address _newImplementationAddress) external onlyOwner {
        require(_newImplementationAddress != address(0),"Invalid address");

        ImplementationAddress = _newImplementationAddress;            
        Emitter(emitterAddress).changeImplementationAddress(_newImplementationAddress);
        
    }

    /// @dev Function to create proxies and initialization of Token and Governor contract
    function createDAO(DAO calldata _params)external returns(address) {
        bytes memory data = abi.encodeWithSignature("initializeERC20(string,string,uint256,uint256,uint256,uint256,uint256,bool,uint256,uint256,address,address,address,address[])",_params.DaoName,_params.DaoSymbol,_params.totalRaiseAmount,_params.minDepositPerUser,_params.maxDepositPerUser,_params.ownerFeePerDeposit,_params.depositFunctioningDays,_params.feeUSDC,_params.quorum,_params.threshold,_params.gnosisAddress,USDCAddress,emitterAddress,_params.daoAdmins);
        address owner = owner();
        address proxy = address(new ProxyContract(ImplementationAddress,owner,data));
        Emitter(emitterAddress).createDao(_params.gnosisAddress, proxy, _params.DaoName, _params.DaoSymbol, _params.totalRaiseAmount, _params.minDepositPerUser, _params.maxDepositPerUser, _params.ownerFeePerDeposit, _params.depositFunctioningDays, _params.feeUSDC, _params.quorum, _params.threshold, emitterAddress,USDCAddress,_params.daoAdmins);
        return proxy;
    }
